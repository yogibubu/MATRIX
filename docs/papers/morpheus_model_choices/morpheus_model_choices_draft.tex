...
each GIC.  A GIC is assigned to a chemical class only when one primitive family is
dominant; classes are never allowed to mix stretches, bends, torsions, or other
coordinate types.  Suggested class names are chemically explicit, for example
\texttt{CC\_stretches}, \texttt{CO\_stretches}, \texttt{CCO\_bends}, or
\texttt{HO\_stretches}.  When hydrogen substitutions are absent, dominant X--H angle
directions may be proposed as fixed directions.  The advisor is deliberately a model
builder, not a black-box guarantee: its output is validated by rank, condition number,
residuals, propagated uncertainties, and warning diagnostics.
...
\input{generated_geometry_tables.tex}

\section{Open developments}
The next improvements should make the advisor more chemically aware without making it
opaque.  Useful extensions are automatic predicate-width selection from reference level,
synthon-aware class splitting, explicit data-budget optimization, and reporting of why
each suggested class was accepted or rejected.  The final paper should also include
examples where the advisor chooses different models as isotopic coverage is increased,
because that directly demonstrates that MORPHEUS formalizes model construction rather
than merely automating a fit.
