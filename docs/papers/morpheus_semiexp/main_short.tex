...
classes reduce the parameter space by enforcing shared corrections.
...
Z-matrices remain compact for tree-like molecules, but rings, fused systems,
and bridged frameworks expose their reference-tree dependence: equivalent
distances, angles, or torsions can become primary or derived depending on the
chosen path, with consequences for covariance propagation. GICs are a flexible
alternative and are widely used in optimization and vibrational
analysis,\cite{Wilson1955,PulayFogarasi1992,Peng1996Redundant,Baker1996Delocalized,BakkenHelgaker2002,Marenich2025GIC}
but are usually redundant, delocalized, or user-defined. Here, they are
generated from topology-based primitive coordinates, reduced to a
non-redundant space, assigned to irreducible representations, and accessed
through primitive-coordinate constraints. Kisiel's \texttt{STRFIT}/PROSPE
...
Norcamphor & 48 GICs; C$_1$ & 18 & 30 hydrogen-frame constraints; 58 Kraitchman predicates & constrained H frames \\
\bottomrule
\end{tabular}
\end{table*}
...
combined in one reproducible protocol. The $\Delta B_\mathrm{vib}$ terms for
this case were generated with the finite-difference-of-gradients workflow of
Ref.~\citenum{Mendolicchio2025FastDvib}; the present work uses them as
documented external reference data rather than introducing a new vibrational
correction protocol.
