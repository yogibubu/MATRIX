...
is maintained where required for common MATRIX services, especially
...
A complete standalone \xyzin{} input can also contain the \texttt{\#MORPHEUS}
run section and be passed directly to the command line:
...
\noindent
In that mode the initial Cartesian XYZ block and \texttt{\#ISOTOPOLOGUES}
provide the molecular and spectroscopic input.  The \texttt{\#MORPHEUS}
section selects the active coordinate model and optional predicates:
...
PREDICATES = INITIAL_GEOMETRY REFERENCE_LEVEL=medium DISTANCE_SIGMA=AUTO ANGLE_SIGMA=AUTO DIHEDRAL_SIGMA=AUTO
PREDICATE_SCOPE = xy_bonds,xh_bonds,heavy_angles,coh_angles,ring_torsions
PRIMITIVE_CLASS_ADVISOR = AUTO_SYNTHON INCLUDE=bonds,angles MIN_GROUP_SIZE=2
SYNTHON_LEVEL = auto
...
\noindent
\texttt{COORDINATE\_MODEL} and \texttt{FIT\_COORDINATES} accept
\texttt{gic} or \texttt{cartesian\_symmetry}.  \texttt{COMPONENTS} may be
written as \texttt{ABC}, \texttt{AB}, \texttt{AC}, \texttt{BC} or in moment
notation such as \texttt{Ia,Ib,Ic}.  \texttt{PREDICATES=INITIAL\_GEOMETRY}
generates weighted primitive predicates centered on the initial Cartesian
geometry using the requested scope and sigmas.  If a sigma is \texttt{AUTO},
MORPHEUS selects it from \texttt{REFERENCE\_LEVEL}: \texttt{high},
\texttt{medium}, \texttt{low}, or \texttt{kraitchman}.  These are input priors,
not output diagnostics.

\texttt{xy\_bonds} denotes bonds between non-hydrogen atoms, while
\texttt{xh\_bonds} denotes any X--H bond; the legacy \texttt{heavy\_bonds} and
\texttt{oh\_bonds} scopes remain accepted but new inputs should use the
general XY/XH names.  \texttt{PRIMITIVE\_CLASS\_ADVISOR=AUTO\_SYNTHON} builds bond and angle
primitive classes from MATRIX topology and atomic synthons.  The synthon
descriptors are continuous; \texttt{SYNTHON\_LEVEL=coarse}, \texttt{medium},
or \texttt{fine} changes the quantization thresholds for covalency,
delocalization, strain, and bond order.  With \texttt{SYNTHON\_LEVEL=auto},
MORPHEUS runs a short internal scan over \texttt{coarse}, \texttt{medium}, and
\texttt{fine}; the candidate results are written under
\texttt{\_synthon\_auto\_candidates}.  The selected level is the simplest model
that satisfies rank and propagated-uncertainty checks for XY bonds,
X--H bonds, and heavy-atom angles.  Non-CH X--H bonds use the same threshold as
XY bonds; C--H bonds use a wider threshold because C--D substitutions are often
absent and C--H distances are therefore less directly determined.  The advisor maps the resulting primitive classes
onto the actual reduced GICs, freezes unsupported directions, limits the number
of active classes to the experimental data budget when
\texttt{PRIMITIVE\_CLASS\_BUDGET=auto}, and prints explicit accept/reject
decision lines.  Explicit user classes remain available through
\texttt{PRIMITIVE\_CLASS = name:R(1,2)|R(2,3)} entries.
...
\midrule
XY distance & bond between two non-H atoms & 0.002~\AA{} \\
non-CH X--H distance & X--H bond with X different from C & 0.002~\AA{} \\
C--H distance & C--H bond & 0.005~\AA{} \\
heavy-atom angle & valence angle without H atoms & 0.2$^\circ$ \\
\bottomrule
\end{tabular}
\end{center}

The broader C--H threshold is not a special rule for sugars.  It reflects the
general information content of many rotational datasets: heavy-atom isotopic
substitution constrains the heavy skeleton much more directly than C--H distances,
and deuterated isotopologues are often absent.  Non-CH X--H bonds are kept with
the tighter XY threshold unless the user explicitly chooses a different predicate
or class policy.
...
information.  It is a conservative tuning layer on top of a chemically valid
model, not an automatic replacement for that model.  A run requested with
\texttt{--apply-sensitivity-advisor} is accepted only if the base model already
passes the MORPHEUS rank and validation checks and the advisor candidate does
not worsen the rotational residuals, rank, conditioning or fitted geometry
beyond the configured gate thresholds.  If the base model is underdetermined,
the advisor report is still written for diagnosis, but its suggestions are not
applied automatically; the user must first add justified predicates, parameter
classes or fixed coordinates.  Typical actions are:
...
\noindent
The validation commands check the canonical MATRIX container before a
...
\begin{enumerate}
  \item create or load a MATRIX project;
