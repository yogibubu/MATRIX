...
\program{} consumes the topology written by MATRIX preprocessing; it does not
own a private graph parser.  The shared \texttt{\#TOPOLOGY} section, schema
\texttt{oracle.xyz.topology.v1}, supplies one-based bonds, normalized bond
orders, elementary rings and aromaticity annotations.  Gaussian Mayer bond
orders are used when present; otherwise MATRIX writes continuous Pauling bond
orders from the shared topology model.  The graph is checked before coordinate
construction: symbols and atomic numbers must be consistent, atom indexes must
be unique, and obvious nonbonded H--H contacts or other impossible short
contacts are treated as input/topology errors.  The graph is then frozen for
the run.  Later refinement or optimization modules must not allow a geometry
step to change this graph silently.

This ownership boundary is part of the GIC contract.  NEO may report how
topology was used, for example which ring bond orders changed the
ring-puckering flexibility factors, but it must not reconstruct another graph
inside GIC construction.  GF, MORPHEUS/SEfit and anharmonic tools consume the
frozen \texttt{\#GIC} and B matrix produced from that same topology.
...
Weak complexes use an explicit topology policy.  In
\texttt{SPECIAL\_COORDINATES} mode fragments remain separate rigid bodies and
NEO adds protected inter-fragment distances, center--atom distances,
translations and orientation coordinates.  In \texttt{PSEUDO\_BONDS} mode
selected contacts, especially hydrogen bonds, are inserted into the construction
graph before ordinary primitive generation; they produce standard stretches,
bends and torsions, but they must not create artificial rings.
...
\subsection{Closure report}

Every build can produce a human-readable report with a \texttt{Closure Summary}
section.  This section is the operational checklist for a production GIC
definition: rank completeness, symmetry method and status, total-symmetric
irrep and count, protected special-coordinate families, ring diagnostics, SALC
coefficient-vector count, maximum SALC normalization residual, and numbers of
singular or dependent rows skipped during reduction.  A production coordinate
set is considered closed only when the rank equals the target vibrational rank,
no singular row is selected, symmetry is either applied or explicitly not
requested, and all nontrivial SALC coefficient vectors are normalized.

The report also prints \texttt{Ring Puckering Diagnostics}.  Each line records
the ring atom list, the endocyclic central bonds used by the \texttt{RPck}
dihedral source space, their normalized bond orders and the final flexibility
factors.  This is the place to inspect whether a fused, spiro or bridged ring
has been treated as intended before the same frozen GIC definition is used by
Gaussian, GF or MORPHEUS.

For regression work, \program{} also provides a compact SALC snapshot command:
\texttt{matrix gicforge salc-snapshot molecule.xyzin salc\_snapshot.json}.  The
snapshot stores a stable hash of the full nontrivial SALC coefficient list and
a small readable subset of representative vectors.  It is the golden test for
the explicit symmetry-coordinate form.  The Fortran audit remains the golden
test for rank, Wilson-B row space and Python/Fortran equivalence, while
Gaussian ReadAllGIC examples test external optimizer compatibility.
...
selected weak contact.  If the weak-contact graph plus one short admissible
closure defines a physical pseudo-cycle, \program{} adds the same number of
intra-cycle components used for ordinary rings: for a cycle of size \(N\), \(N\)
distances, \(N-3\) angular combinations and \(N-3\) torsional/puckering
combinations.  Long-range closures are rejected.  Primitive candidates are
still reduced by analytic B-matrix rank; the pseudo-bond is an
internal-coordinate edge, not a request to keep every primitive generated around
it.
