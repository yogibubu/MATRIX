...
rows are analytic linear combinations of dihedral B rows.  For isolated rings,
the Gaussian block can write those \texttt{RPck} components as inactive
auxiliaries and derive active \texttt{QPck} and \texttt{PhiP} functionals from
consecutive \texttt{RPck} pairs.  For fused rings, the selected
\texttt{RPck} components remain the active exported coordinates and are
symmetrized directly; the code does not collapse the fused-ring puckering
space into polar coordinates for individual rings.
...
frozen ORACLE B matrix uses these linear components directly.  For isolated
rings in Gaussian ReadGIC export, consecutive selected \texttt{RPck} pairs are
written as inactive auxiliaries and converted to active puckering coordinates
...
primitive basis is endocyclic torsional, but the Gaussian-active coordinates are
the independent \(q\)- and \(\phi\)-like ring deformation coordinates derived
from selected \texttt{RPck} pairs.  If an odd component remains after pairing,
it stays as an inactive \texttt{RPck} auxiliary.  Fused rings are treated
differently: each elementary fused-ring cycle contributes its own protected
ring-puckering source space, the fused set is reduced only within the protected
ring family, and the surviving \texttt{RPck} components are exported directly.
...
linear analytic B-matrix evaluation; isolated rings may additionally export
active \texttt{QPck}/\texttt{PhiP} functionals for complete pairs, while fused
rings keep the selected \texttt{RPck} rows active. \\
