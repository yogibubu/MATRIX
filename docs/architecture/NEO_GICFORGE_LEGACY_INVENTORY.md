...
- The MATRIX-native backend generates primitive candidates from the saved
...
- Ring-specialized coordinate family tagging is now present in MATRIX-native
...
- `matrix_engines.run_legacy_gicforge` is the executable harness for the
...
2. Freeze `#SYCART` as an external-mode-free Cartesian basis for the same
   validated molecule.
3. Route `matrix gicforge build` and Gaussian input generation through those
   frozen sections.
