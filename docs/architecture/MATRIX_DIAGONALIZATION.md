# MATRIX Shared Diagonalization

MATRIX keeps dense Hermitian diagonalization behind one shared API:

```python
from matrix_core import diagonalize_hermitian, eigh_arrays, eigvalsh_array
```
...
`MATRIX_DIAGONALIZER_STRICT_GPU=1` makes GPU failures fatal. With the default
value, a failed GPU call falls back to SciPy CPU so production workflows keep
running.

`matrix-set` does not install CuPy or PyTorch automatically. GPU packages are
...
now it stays in `matrix-set` to preserve the validated WMS-Rot behavior.
