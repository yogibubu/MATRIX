# ADR-0006: Validation Gate Before NEO
...
After LINK has imported a molecule, determined symmetry, built
...
`matrix-chem` owns `#VALIDATION` with schema `oracle.xyz.validation.v1`.
...
`matrix-neo` must require `#VALIDATION` with `STATUS PASS` before writing
