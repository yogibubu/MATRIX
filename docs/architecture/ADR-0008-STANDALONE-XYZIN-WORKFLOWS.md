...
MATRIX tools are not only steps inside a single GUI/pipeline. SEFit/MORPHEUS,
...
Every scientific MATRIX package must expose a standalone service and CLI path
that accepts an enriched XYZ/`xyzin` file directly.

The current standalone surface is recorded in `matrix_core.tool_contracts` and
is visible with:

```bash
python -m matrix contracts
python -m matrix contracts --format markdown
python -m matrix contracts --tool NEO
python -m matrix contracts --tool gf --check-xyzin molecule.xyzin
```

Standalone tools must:

- use `matrix-core` section APIs;
...
- optionally write manifests when a workspace/run directory is supplied, but
  not require a full MATRIX project workspace.
...
- `matrix-rovib summarize` reads `#BASIC`, `#ROTATIONAL` and `#VIBRATIONAL`
...
- DVR writes and reads `#DVR` request/manifest/output state. Gaussian scan logs
  remain adapter inputs for `matrix dvr prepare`, not private parsers inside
...
- TRINITY reads `#TRINITY` as its external energy/gradient geometry-optimization
  ...
  `matrix trinity prepare`; the future optimizer loop must consume that section
...
- Existing ORACLE fixtures that already contain enough sections remain useful
  regression tests.
- LINK and validation are recommended producers of clean state, but
...
- DVR post-run collection is owned by `matrix-dvr`; GUI code consumes the
