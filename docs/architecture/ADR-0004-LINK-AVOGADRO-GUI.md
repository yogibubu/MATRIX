# ADR 0004: LINK, Avogadro And GUI Visualization

Date: 2026-06-26

## Decision

The MATRIX preprocessing layer is LINK, a normalized external-source adapter.
It is the only
...
MATRIX metadata are stored after the first XYZ frame in named sections.
...
LCB25 is a remote LINK source. PCS2, SE and HPCS2 XYZ archives can be
...
Avogadro and imports it back, MATRIX replaces only the first XYZ block and
preserves metadata sections. If geometry changes, cached sections that depend on
geometry must be marked stale or regenerated by LINK.
...
- LINK is the only place that turns external formats into MATRIX state. The
  current implementation package is still `matrix-link` for compatibility.
