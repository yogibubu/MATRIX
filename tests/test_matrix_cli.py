...
from matrix_core import cli as matrix_cli
from matrix_gf import read_gf_ped_section
...
def test_matrix_python_module_entrypoint_prints_help():
    result = subprocess.run(
        [sys.executable, "-m", "matrix", "--help"],
        cwd=ROOT,
        env=os.environ.copy(),
        check=True,
        capture_output=True,
    ...
    assert "MATRIX workflow CLI" in result.stdout
    assert "link" in result.stdout
    assert "babel" in result.stdout
    assert "gicforge" in result.stdout


def test_oracle_python_module_entrypoint_remains_compatibility_alias():
    result = subprocess.run(
    ...
    assert "usage: oracle" in result.stdout
    assert "MATRIX workflow CLI" in result.stdout
...
def test_validate_cli_calls_writer(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("matrix_chem.write_validation_section", fake_validate)

    rc = matrix_run.main(["validate", str(path), "--require-fragments"])
...
def test_lcb25_fetch_cli_calls_sync(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("matrix_link.sync_lcb25_library", fake_sync)
...
def test_contracts_cli_lists_standalone_tool_registry(capsys):
    rc = matrix_run.main(["contracts", "--tool", "NEO"])
    out = capsys.readouterr().out

    assert rc == 0
    assert "gicforge: NEO / GICForge" in out
...
def test_contracts_cli_json_can_omit_gui(capsys):
    rc = matrix_run.main(["contracts", "--format", "json", "--no-gui"])
    out = capsys.readouterr().out
...
def test_contracts_cli_prints_framework_acronym(capsys):
    rc = matrix_run.main(["contracts", "--framework"])
    out = capsys.readouterr().out
...
def test_matrix_cli_alias_prints_framework_acronym(capsys):
    rc = matrix_cli.matrix_main(["contracts", "--framework"])
    out = capsys.readouterr().out
...
def test_contracts_cli_checks_xyzin_readiness(tmp_path, capsys):
    ...
    rc = matrix_run.main(["contracts", "--tool", "thermo", "--check-xyzin", str(xyzin)])
    out = capsys.readouterr().out
    assert rc == 0
    assert "thermo: ready=1 missing=none" in out

    rc = matrix_run.main(["contracts", "--tool", "gf", "--check-xyzin", str(xyzin)])
    out = capsys.readouterr().out
    assert rc == 2
...
def test_link_preprocess_cli_calls_shared_pipeline(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("matrix_chem.preprocess_to_enriched_xyz", fake_preprocess)
    ...
    assert "Preprocessed LINK molecule" in capsys.readouterr().out


def test_babel_preprocess_cli_remains_compatibility_alias(tmp_path, monkeypatch, capsys):
    calls = {}
    source = tmp_path / "source.inp"
    output = tmp_path / "molecule.xyzin"
    ...
    def fake_preprocess(target_source, target_output, *, source_kind="auto", symmetry_thresholds):
        ...
        return SimpleNamespace(
            path=target_output,
            geometry=SimpleNamespace(natoms=1),
            point_group="C1",
            topology_bond_count=0,
            ring_count=0,
        )

    monkeypatch.setattr("matrix_chem.preprocess_to_enriched_xyz", fake_preprocess)

    rc = matrix_run.main(["babel", "preprocess", str(source), str(output)])
...
def test_link_console_alias_calls_preprocess(tmp_path, monkeypatch, capsys):
    source = tmp_path / "source.inp"
    output = tmp_path / "molecule.xyzin"
    ...
    def fake_preprocess(target_source, target_output, *, source_kind="auto", symmetry_thresholds):
        return SimpleNamespace(
            path=target_output,
            geometry=SimpleNamespace(natoms=2),
            point_group="Cs",
    ...
    rc = matrix_cli.link_main(["preprocess", str(source), str(output)])
...
def test_gaussian_summary_cli_prints_log_markers(tmp_path, capsys):
    ...
    rc = matrix_run.main(["gaussian", "summary", str(log)])
    out = capsys.readouterr().out
...
def test_gaussian_status_cli_calls_adapter(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("matrix_gaussian.gaussian_job_status", fake_status)

    rc = matrix_run.main(["gaussian", "status", str(tmp_path)])
    out = capsys.readouterr().out
...
def test_gaussian_run_cli_calls_adapter(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("matrix_gaussian.run_gaussian_job", fake_run)
...
def test_gaussian_formchk_cli_calls_adapter(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("matrix_gaussian.formchk_checkpoint", fake_formchk)
...
def test_gaussian_fchk_summary_cli_calls_qff_reader(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("matrix_gaussian.read_gaussian_fchk_qff", fake_read)

    rc = matrix_run.main(["gaussian", "fchk-summary", str(fchk)])
    out = capsys.readouterr().out
...
def test_gaussian_promote_fchk_cli_calls_adapter(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("matrix_gaussian.promote_gaussian_fchk_to_xyzin", fake_promote)
...
def test_gaussian_promote_electronic_cli_calls_adapter(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("matrix_gaussian.promote_gaussian_electronic_log_to_xyzin", fake_promote)
...
def test_gaussian_promote_rovib_cli_calls_adapter(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("matrix_gaussian.promote_gaussian_rovib_to_xyzin", fake_promote)
...
def test_gf_cli_can_use_hessian_from_xyzin_without_fchk(tmp_path, monkeypatch, capsys):
    ...
    rc = matrix_run.main(["gf", "--xyzin", str(xyzin), "--local", "--symmetry-blocks"])
    out = capsys.readouterr().out
...
def test_vpt2_vci_cli_uses_qff_section_from_xyzin(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("matrix_vpt2_vci.run_vpt2_vci_report", fake_run)
    monkeypatch.setattr("matrix_vpt2_vci.write_csv_tables", fake_csv)
...
def test_vpt2_vci_cli_writes_manifest_and_xyzin_section(tmp_path, monkeypatch, capsys):
    from matrix_vpt2_vci import read_vpt2_vci_section
...
def test_vpt2_vci_collect_cli_refreshes_section(tmp_path, capsys):
    ...
    rc = matrix_run.main(["vpt2-vci", "--collect", str(xyzin)])
    out = capsys.readouterr().out
    section = read_vpt2_vci_section(xyzin)
...
def test_dvr_prepare_cli_writes_manifest_and_xyzin_section(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("matrix_dvr.write_dvr_manifest", fake_manifest)
...
def test_dvr_collect_cli_refreshes_post_run_outputs(tmp_path, capsys):
    from matrix_dvr import DVRRequest, dvr_section_from_request, read_dvr_section, write_dvr_section
    ...
    rc = matrix_run.main(["dvr", "collect", str(xyzin)])
    out = capsys.readouterr().out
...
def test_dvr_run_cli_uses_xyzin_request_and_collects_outputs(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("matrix_dvr.run_dvr_request", fake_run)

    rc = matrix_run.main(["dvr", "run", "--xyzin", str(xyzin), "--timeout", "3"])
    out = capsys.readouterr().out
    section = read_dvr_section(xyzin)
...
def test_dvr_run_cli_builds_log_request_and_updates_xyzin(tmp_path, monkeypatch, capsys):
    from matrix_dvr import read_dvr_section
...
def test_fragments_plan_cli_calls_writer(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("matrix_fragments.write_fragment_plan_section", fake_write)

    rc = matrix_run.main(["fragments", "plan", str(path)])

    assert rc == 0
    assert calls == {"target": path}
    assert "Planned MATRIX fragment workflow" in capsys.readouterr().out
...
def test_fragments_build_cli_calls_writer(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("matrix_fragments.write_fragment_build_section", fake_write)

    rc = matrix_run.main(["fragments", "build", str(path)])

    assert rc == 0
    assert calls == {"target": path}
    assert "Built MATRIX fragments" in capsys.readouterr().out
...
def test_fragments_centers_cli_calls_writer(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("matrix_fragments.write_interaction_center_section", fake_write)

    rc = matrix_run.main(["fragments", "centers", str(path)])

    assert rc == 0
    assert calls == {"target": path}
    assert "Built MATRIX interaction centers" in capsys.readouterr().out
...
def test_rovib_summarize_cli_prints_summary(tmp_path, capsys):
    ...
    rc = matrix_run.main(["rovib", "summarize", str(path)])
...
def test_rovib_wmsrot_run_cli_calls_local_engine_writer(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("matrix_rovib.write_wmsrot_spectrum_csv", fake_write)
...
def test_thermo_cli_calls_shared_pipeline(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("matrix_thermo.run_thermo_on_xyzin", fake_run)
    ...
    out = capsys.readouterr().out
    assert "Ran MATRIX Thermo" in out
...
def test_gicforge_build_cli_calls_writer(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("matrix_neo.write_gicforge_build_sections", fake_write)

    rc = matrix_run.main(["gicforge", "build", str(path), "--symmetrize", "--sycart"])
...
def test_neo_cli_alias_calls_gicforge_build_writer(tmp_path, monkeypatch, capsys):
    ...
    rc = matrix_run.main(["neo", "build", str(path), "--symmetrize"])
...
def test_gicforge_cli_passes_improper_dihedrals_flag(tmp_path, monkeypatch, capsys):
    ...
    rc_plan = matrix_run.main(["gicforge", "plan", str(path), "--improper-dihedrals"])
    rc_build = matrix_run.main(["gicforge", "build", str(path), "--improper-dihedrals"])
...
def test_gf_cli_runs_xyzin_report_and_csv_export(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("matrix_gf.run_xyzin_gf_report_from_fchk", fake_run)
    monkeypatch.setattr("matrix_gf.write_csv_tables", fake_csv)
...
def test_gicforge_bmatrix_cli_calls_writer(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("matrix_neo.write_gic_b_matrix", fake_write)

    rc = matrix_run.main(["gicforge", "bmatrix", str(path), str(output)])
...
def test_gicforge_report_cli_calls_writer(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("matrix_neo.write_gic_report", fake_write)

    rc = matrix_run.main(["gicforge", "report", str(path), str(output)])
...
def test_gicforge_corpus_cli_prints_inventory(capsys):
    rc = matrix_run.main(["gicforge", "corpus", "--root", str(GIC_CORPUS)])
...
def test_gicforge_corpus_audit_cli_prints_parser_budget(capsys):
    rc = matrix_run.main(["gicforge", "corpus-audit", "--root", str(GIC_CORPUS)])
...
def test_gicforge_fortran_audit_cli_prints_summary(tmp_path, monkeypatch, capsys):
    from matrix_neo import GICForgeFortranAudit, GICForgeFortranAuditResult
    ...
    monkeypatch.setattr("matrix_neo.audit_gicforge_fortran_corpus", fake_audit)
...
def test_semiexp_benchmark_cli_calls_paper_artifact_generator(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("matrix_morpheus.generate_paper_benchmark_artifacts", fake_generate)
...
def test_semiexp_ensemble_paper_cli_calls_artifact_writer(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("matrix_morpheus.write_ensemble_jpcl_artifacts", fake_write)
...
def test_semiexp_ensemble_scan_cli_calls_scan_helpers(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("matrix_morpheus.run_ensemble_prior_scan", fake_prior)
    monkeypatch.setattr("matrix_morpheus.run_ensemble_synthon_threshold_scan", fake_synthon)

    rc_prior = matrix_run.main(
        [
...
def test_multistructure_reference_search_cli_calls_library(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("matrix_morpheus.search_reference_library", fake_search)
...
def test_multistructure_build_reference_geometry_cli_calls_library(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("matrix_morpheus.build_reference_assisted_geometry", fake_build)
...
def test_gicforge_gaussian_input_cli_calls_writer(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("matrix_neo.write_gicforge_gaussian_input", fake_write)
