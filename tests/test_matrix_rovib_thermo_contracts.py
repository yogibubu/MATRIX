...
from matrix_qm import normal_modes_section_from_arrays, write_normal_modes_section
...
def test_wmsrot_vendor_engine_routes_eigh_through_matrix_diagonalizer(monkeypatch):
    import numpy as np

    from matrix_rovib.vendor import wmsrot_engine
    ...
    monkeypatch.setattr(wmsrot_engine, "_matrix_eigh_arrays", fake_eigh)

    values, vectors = wmsrot_engine._matrix_eigh(np.diag([2.0, 1.0]))
...
def test_nist_jcamp_parser_and_gas_phase_download_policy(tmp_path, monkeypatch):
    from matrix_rovib import vibspec
