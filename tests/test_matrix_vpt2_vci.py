...
from matrix_gf import gf_from_gaussian_fchk_with_matrix_gics, gf_from_hessian_input_with_matrix_gics
...
from matrix_vpt2_vci.gaussian_qff import hessian_input_from_gaussian_fchk
...
def test_vci_dense_solver_routes_diagonalization_through_matrix_core(monkeypatch):
    from matrix_vpt2_vci import vci
...
def test_gf_from_gaussian_cartesian_hessian_uses_oracle_nonredundant_gics():
    ...
    result = gf_from_hessian_input_with_matrix_gics(canonical)
    adapter_result = gf_from_gaussian_fchk_with_matrix_gics(FCHK)
...
def test_vpt2_vci_fortran_inventory_uses_matrix_engines_layout():
...
def _write_vpt2_vci_outputs(run_dir: Path) -> None:
    ...
    (run_dir / "vpt2_vci.report").write_text(
        "VPT2/VCI comparison on canonical MATRIX QFF\n",
        encoding="utf-8",
    )
